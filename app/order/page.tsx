...
export default function OrderPage() {
  ...
  const isFormValid = name && mobile && /^02\d{7,9}$/.test(mobile) && pickupTime && cart.length > 0

  const placeOrder = async () => {
    if (!isFormValid) {
      alert('Please fill in all required fields correctly and add items to your cart.')
      return
    }

    // Here you would typically send the order to your backend
    console.log('Order placed:', { name, mobile, email, orderType, paymentMethod, pickupTime, cart, subtotal })

    // Simulate sending notifications
    console.log('Sending confirmation to customer:', mobile)
    console.log('Sending WhatsApp notification to restaurant:', RESTAURANT_PHONE)
    console.log('Sending SMS notification to restaurant:', RESTAURANT_PHONE)
    ...
    alert('Order placed successfully! You will receive a confirmation shortly. The restaurant has been notified of your order.')
  }
    ...
    <div className="min-h-screen bg-cover bg-center bg-no-repeat flex flex-col" style={{backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url('https://hebbkx1anhila5yf.public.blob.vercel-storage.com/FInal%20background-1QBTDWqBZCuDtvk4nL5549cN20arkv.jpeg')"}}>
      ...
      <main className="flex-grow container mx-auto px-4 py-8 flex flex-col lg:flex-row bg-white bg-opacity-90 my-8 rounded-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {category.items.map((item, index) => (
                    <div key={index} className="border p-4 rounded-lg bg-white shadow-md">
                      ...
                      <Button onClick={() => addToCart(item)} className="mt-2 bg-purple-600 hover:bg-purple-700 text-white">Add to Cart</Button>
                    </div>
                  ))}
                </div>
              </TabsContent>
        ...
        <div className="w-full lg:w-1/3">
          ...
          <form className="mt-6 space-y-4" onSubmit={(e) => { e.preventDefault(); placeOrder(); }}>
            <div>
              <Label htmlFor="name">Name *</Label>
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} required />
            </div>
            <div>
              <Label htmlFor="mobile">Mobile Number (NZ) *</Label>
              <Input id="mobile" value={mobile} onChange={(e) => setMobile(e.target.value)} required pattern="^02\d{7,9}$" />
            </div>
            <div>
            ...
            <Button type="submit" disabled={!isFormValid} className="w-full bg-purple-600 hover:bg-purple-700 text-white">
              Place Order
            </Button>
          </form>
        </div>
