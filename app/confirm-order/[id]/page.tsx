...
export default function ConfirmOrderPage({ order }: { order: Order }) {
  ...
  const handleButtonClick = async (action: string, value: string) => {
    ...
    if (action === 'preparationTime') {
      await fetch('/api/set-preparation-time', {
        method: 'POST',
      ...
      await fetch('/api/update-order', {
        ...
        body: JSON.stringify({ 
          orderId: order.id, 
          status: value, 
          preparationTime: order.preparationTime || 'Req. Time' 
        }),
      })
    }
  }
          ...
          <div className="grid grid-cols-4 gap-2 mb-6">
            {preparationTimes.map((time) => (
              <Button
                ...
                className={`w-full ${time === order.preparationTime ? 'bg-[#4CAF50] text-white' : 'bg-white text-black'} hover:bg-[#45a049] hover:text-white border border-gray-200 rounded ${animatingButton === 'preparationTime' + time ? 'scale-95' : 'scale-100'} transition-transform duration-300`}
              >
                {time}
              </Button>
            ))}
