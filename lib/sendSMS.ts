...
function sanitizeMessage(message: string): string {
  // Remove or replace problematic URLs
  return message.replace(/https?:\/\/\S+/g, '[URL removed due to restrictions]')
}
...
export async function sendSMS(to: string, message: string): Promise<void> {
  ...
  if (!formattedPhone || !sanitizedMessage || !CLICKSEND_USERNAME || !CLICKSEND_API_KEY) {
    console.error('Missing required parameters for SMS sending:', { 
      hasPhone: !!formattedPhone, 
